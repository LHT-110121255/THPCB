...
// Giới hạn/heuristics
const MAX_CONSEC_SAME_SUBJECT_PER_SESSION = 2;
const MAX_SAME_SUBJECT_PER_DAY = 4;
const REQUIRE_FRIDAY_MORNING = true;
const BLOCKY_SUBJECTS = new Set(['HĐTN']);
...
function distributeDays(blocksPerSubject, allDays) {
  ...
  for (const [subjectName, blocks] of blocksPerSubject.entries()) {
    ...
    let desiredDays = Math.min(allDays.length, Math.max(1, Math.ceil(nBlocks / 2)));
    if (subjectName === 'Tiếng Việt') desiredDays = Math.min(allDays.length, Math.max(desiredDays, 4));
...
class AutoAssign {
  Handle = async (req, res) => {
    try {
      ...
      // Build blocks
      const blocks = [];
      const blocksPerSubject = new Map();

      for (const d of demands) {
        let remain = d.periods;
        const pushBlock = (size) => {
          const b = { size, subjectName: d.subjectName, subjectId: d.subjectId, classId: d.classId, className: d.className, teacher: d.teacher, classDistance: d.classDistance };
          blocks.push(b);
          if (!blocksPerSubject.has(d.subjectName)) blocksPerSubject.set(d.subjectName, []);
          blocksPerSubject.get(d.subjectName).push(b);
        };
        ...
        if (DOUBLE_PERIOD_SUBJECTS.includes(d.subjectName)) {
          ...
          while (remain >= 2) { pushBlock(2); remain -= 2; }
          if (remain === 1) { pushBlock(1); remain = 0; }
        } else while (remain > 0) { pushBlock(1); remain--; }
      }

      const planBySubject = distributeDays(blocksPerSubject, DAYS);
      const allSlots = buildAllSlots();
      const slotsByDaySession = new Map();
      for (const s of allSlots) {
        const k = daySessionKey(s);
        if (!slotsByDaySession.has(k)) slotsByDaySession.set(k, []);
        slotsByDaySession.get(k).push(s);
      }
      for (const [k, arr] of slotsByDaySession) arr.sort((a, b) => a.period - b.period);

      const occupiedClass = new Map();
      const occupiedTeacher = new Map();
      const subjectCountPerDay = new Map();
      const subjectRunPerSession = new Map();
      const teacherSlotsByDay = new Map();

      const isFreeForClass = (classId, s) => !(occupiedClass.get(String(classId))?.get(slotKey(s)));
      const isFreeForTeacher = (teacher, s) => !(occupiedTeacher.get(teacher)?.get(slotKey(s)));
      const getDaySubjectCount = (classId, s, subject) => { const k = `${classId}-${s.day}`; const m = subjectCountPerDay.get(k); return m?.get(subject) || 0; };
      const incDaySubjectCount = (classId, s, subject) => { const k = `${classId}-${s.day}`; if (!subjectCountPerDay.has(k)) subjectCountPerDay.set(k, new Map()); const m = subjectCountPerDay.get(k); m.set(subject, (m.get(subject) || 0) + 1); };
      const getRunInfo = (classId, s) => { const k = `${classId}-${s.day}-${s.session}`; if (!subjectRunPerSession.has(k)) subjectRunPerSession.set(k, { lastSubject: null, len: 0, byPeriod: new Map() }); return subjectRunPerSession.get(k); };
      const teacherDaySetKey = (s) => `${s.session}-${s.period}`;
      const occupy = (classId, teacher, s, subjectName) => { ensureMapOfMaps(occupiedClass, String(classId), slotKey(s), true); ensureMapOfMaps(occupiedTeacher, teacher, slotKey(s), true); incDaySubjectCount(classId, s, subjectName); const ri = getRunInfo(classId, s); ri.byPeriod.set(s.period, subjectName); ri.lastSubject === subjectName ? ri.len++ : (ri.lastSubject = subjectName, ri.len = 1); const tKey = `${teacher}-${s.day}`; if (!teacherSlotsByDay.has(tKey)) teacherSlotsByDay.set(tKey, new Set()); teacherSlotsByDay.get(tKey).add(teacherDaySetKey(s)); };
      const teacherHasCloseSlot = (teacher, s, minGap = 1) => { const tKey = `${teacher}-${s.day}`; const set = teacherSlotsByDay.get(tKey); if (!set || set.size === 0) return false; for (const tag of set) { const [session, pStr] = tag.split('-'); const p = Number(pStr); if (session !== s.session) continue; if (Math.abs(p - s.period) < 2) return true; } return false; };
      ...
      function findContiguousFreeRun(classId, teacher, day, session, L, subjectName, classDistance) {
        ...
        for (let i = 0; i + L - 1 < arr.length; i++) {
          ...
          for (const s of win) {
            if (!isFreeForClass(classId, s) || !isFreeForTeacher(teacher, s)) { ok = false; break; }
            if (!canAssignTeacher(teacher, 1)) { ok = false; break; }
            if (classDistance && teacherHasCloseSlot(teacher, s, 1)) { ok = false; break; }
          }
          if (!ok) continue;
          const already = getDaySubjectCount(classId, { day }, subjectName);
          if (already + L > MAX_SAME_SUBJECT_PER_DAY) continue;
          ...
          const prevSlot = arrFull[i - 1];
          let prevSame = 0;
          if (prevSlot && runInfo.byPeriod.get(prevSlot.period) === subjectName) { let p = prevSlot.period; while (runInfo.byPeriod.get(p) === subjectName) { prevSame++; p--; } }
          const nextSlot = arrFull[i + L];
          let nextSame = 0;
          if (nextSlot && runInfo.byPeriod.get(nextSlot.period) === subjectName) { let p = nextSlot.period; while (runInfo.byPeriod.get(p) === subjectName) { nextSame++; p++; } }
          if (prevSame + L > MAX_CONSEC_SAME_SUBJECT_PER_SESSION) continue;
          if (nextSame + L > MAX_CONSEC_SAME_SUBJECT_PER_SESSION) continue;
          if (prevSame + L + nextSame > MAX_CONSEC_SAME_SUBJECT_PER_SESSION) continue;
          return win;
      ...
      const PRIORITY_TEACHERS = ['Trí', 'Thống', 'Thành'];

      blocks.sort((a, b) => {
        if (a.size !== b.size) return b.size - a.size;
        if (a.subjectName === 'Tiếng Việt' && b.subjectName !== 'Tiếng Việt') return -1;
        if (a.subjectName !== 'Tiếng Việt' && b.subjectName === 'Tiếng Việt') return 1;
        const aPri = PRIORITY_TEACHERS.includes(a.teacher) ? 0 : 1;
        const bPri = PRIORITY_TEACHERS.includes(b.teacher) ? 0 : 1;
        return aPri - bPri;
      });

      const subjectPlacedIndex = new Map();
      function pickPlannedDay(subjectName) {
        const idx = subjectPlacedIndex.get(subjectName) || 0;
        const arrDays = planBySubject.get(subjectName) || DAYS;
        const d = arrDays[idx % arrDays.length];
      ...
      function tryPlaceBlock(block) {
        // Ưu tiên block đôi cho Trí
        if (block.teacher === 'Trí' && block.size === 1) {
          const run = findContiguousFreeRun(block.classId, block.teacher, pickPlannedDay(block.subjectName), 'Sáng', 2, block.subjectName, block.classDistance)
                   || findContiguousFreeRun(block.classId, block.teacher, pickPlannedDay(block.subjectName), 'Chiều', 2, block.subjectName, block.classDistance);
          if (run) block.size = 2;
        }

        const intendedDay = pickPlannedDay(block.subjectName);
        const dayOrder = [intendedDay, ...DAYS.filter(d => d !== intendedDay)];
        for (const day of dayOrder) {
          const sessions = day === 6 ? ['Sáng'] : ['Sáng', 'Chiều'];
          for (const session of sessions) {
            const run = findContiguousFreeRun(block.classId, block.teacher, day, session, block.size, block.subjectName, block.classDistance);
            if (!run) continue;
            for (const s of run) {
              created.push({ class: block.classId, subject: block.subjectId, subjectName: block.subjectName, teacher: block.teacher, day: s.day, session: s.session, period: s.period, week });
              occupy(block.classId, block.teacher, s, block.subjectName);
              bumpLoad(block.teacher, 1);
            }
            return true;
      ...
      for (const b of blocks.filter(x => x.size === 2)) {
        ...
        const ok = tryPlaceBlock(b);
        if (!ok) unassigned.push({ className: b.className, subjectName: b.subjectName, teacher: b.teacher, periodsMissing: 2, reason: 'Không tìm được 2 slot liên tiếp hợp lệ' });
      }
      ...
      for (const b of blocks.filter(x => x.size === 1)) {
        ...
        const ok = tryPlaceBlock(b);
        if (!ok) unassigned.push({ className: b.className, subjectName: b.subjectName, teacher: b.teacher, periodsMissing: 1, reason: 'Không tìm được slot hợp lệ' });
      }
      ...
      if (REQUIRE_FRIDAY_MORNING) {
        ...
        for (const classId of classesSet) {
          for (const s of fridaySlots) {
            ...
            unassigned.push({ className: classes.find(c => String(c._id) === String(classId))?.name, subjectName: 'N/A', teacher: 'N/A', periodsMissing: 1, reason: `Thiếu tiết sáng Thứ 6 (tiết ${s.period})` });
          }
        }
      }
      ...
      res.status(200).json({
        ...
        week,
        stats: { slotsCreated: created.length, teachersLoad: Array.from(teacherLoad.entries()).map(([t, n]) => ({ teacher: t, periods: n })), unassigned }
      });
    } catch (err) {
      console.error(err);
      res.status(500).json({ success: false, message: 'Lỗi phân công tự động', error: err.message });
